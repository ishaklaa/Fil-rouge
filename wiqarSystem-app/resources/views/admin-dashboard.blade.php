    ...
    <aside class="w-56 shrink-0 bg-brown-400 flex flex-col h-full shadow-2xl">
        ...
        <nav class="flex-1 px-3 py-4 flex flex-col gap-1 overflow-y-auto scroll-y">
            ...
            <a href="{{ route('admin.dashboard') }}"
               class="nav-link {{ request()->routeIs('admin.dashboard') ? 'active' : '' }} flex items-center gap-3 px-3 py-2.5 rounded-xl text-beige-200 hover:bg-brown-300/50 text-sm font-medium">
            ...
            <a href="{{ route('profiles.index') }}"
               class="nav-link {{ request()->routeIs('profiles.*') ? 'active' : '' }} flex items-center gap-3 px-3 py-2.5 rounded-xl text-beige-200 hover:bg-brown-300/50 text-sm font-medium">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="{{ route('branches.index') }}"
               class="nav-link {{ request()->routeIs('branches.*') ? 'active' : '' }} flex items-center gap-3 px-3 py-2.5 rounded-xl text-beige-200 hover:bg-brown-300/50 text-sm font-medium">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="{{ route('activities.index') }}"
               class="nav-link {{ request()->routeIs('activities.*') ? 'active' : '' }} flex items-center gap-3 px-3 py-2.5 rounded-xl text-beige-200 hover:bg-brown-300/50 text-sm font-medium">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="{{ route('order.history') }}"
               class="nav-link {{ request()->routeIs('order.history') ? 'active' : '' }} flex items-center gap-3 px-3 py-2.5 rounded-xl text-beige-200 hover:bg-brown-300/50 text-sm font-medium">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="{{ route('cashier.statistics') }}"
               class="nav-link {{ request()->routeIs('cashier.statistics') ? 'active' : '' }} flex items-center gap-3 px-3 py-2.5 rounded-xl text-beige-200 hover:bg-brown-300/50 text-sm font-medium">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path d="M3 3v18h18"/>
        ...
        <div class="px-3 py-4 border-t border-brown-300/40 flex flex-col gap-2">
            <div class="flex items-center gap-3 px-3 py-2">
                <div class="w-8 h-8 rounded-lg bg-brown-300 flex items-center justify-center shrink-0">
                    <span class="font-playfair text-sm font-bold text-beige-200">
{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
</span>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-beige-200 text-sm font-medium truncate">{{ Auth::user()->name }}</p>
                    <p class="text-beige-400 text-xs truncate">{{ ucfirst(Auth::user()->role->name ?? 'Admin') }}</p>
                </div>
            </div>
            <a href="/logout"
    ...
    <!-- ── MAIN CONTENT ── -->
    <main class="flex-1 overflow-y-auto scroll-y bg-beige-100 p-6">
        ...
        <div class="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-white rounded-2xl border border-beige-200 shadow-sm p-5 flex flex-col gap-2">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-8 h-8 rounded-xl bg-beige-100 flex items-center justify-center">
                        <svg class="w-4 h-4 text-brown-300" fill="none" stroke="currentColor" stroke-width="2"
                             viewBox="0 0 24 24">
                            <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/>
                            <circle cx="9" cy="7" r="4"/>
                        </svg>
            ...
            <div class="bg-white rounded-2xl border border-beige-200 shadow-sm p-5 flex flex-col gap-2">
                <div class="flex items-center justify-between">
                    <p class="text-xs font-medium text-brown-300 tracking-widests uppercase">Orders Today</p>
                    <div class="w-8 h-8 rounded-xl bg-beige-100 flex items-center justify-center">
                        <svg class="w-4 h-4 text-brown-300" fill="none" stroke="currentColor" stroke-width="2"
                             viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-2xl border border-beige-200 shadow-sm p-5">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('profiles.index') }}"
                   class="text-xs text-brown-200 hover:text-brown-400 transition-all">View all →</a>
            </div>
                ...
                @forelse($recentUsers as $user)
                    <div class="flex items-center gap-3 py-2 border-b border-beige-100 last:border-0">
                        <div class="w-8 h-8 rounded-lg bg-brown-400 flex items-center justify-center shrink-0">
                            <span
                                class="font-playfair text-sm font-bold text-beige-200">{{ strtoupper(substr($user->name, 0, 1)) }}</span>
                        </div>
                        <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            <span class="text-xs text-brown-200">{{ ucfirst($user->role->name ?? '—') }}</span>
