...
<head>
    ...
    <style>
        ...
        @media (max-width: 1023px) {
            ...
            #create-panel-overlay.active { display: block; }
        }
    </style>
</head>
<body class="bg-beige-100 min-h-screen">

<div class="lg:hidden flex items-center justify-between px-4 py-3 bg-brown-400 shadow-lg sticky top-0 z-30">
    ...
    <button onclick="toggleCreatePanel()"
            class="flex items-center gap-1.5 bg-brown-300/50 hover:bg-brown-300 text-beige-200 text-xs font-medium rounded-xl px-3 py-2 transition-all">
...
<div id="create-panel-overlay" onclick="toggleCreatePanel()"></div>
...
<div class="flex md:h-screen md:overflow-hidden">
    ...
    <aside id="sidebar" class="w-56 shrink-0 bg-brown-400 flex flex-col h-full shadow-2xl">
        ...
        <div class="px-5 py-5 border-b border-brown-300/40 md:hidden">
            <p class="text-beige-400 text-xs font-light tracking-widest uppercase">{{ ucfirst(Auth::user()->role->name ?? '') }} Panel</p>
        </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <div class="flex gap-5 p-4 md:p-5 h-full overflow-y-auto lg:overflow-hidden scroll-y">

            <div id="create-panel" class="lg:w-[340px] lg:shrink-0 lg:flex lg:flex-col lg:gap-4 bg-beige-100 lg:bg-transparent lg:static lg:transform-none lg:rounded-none lg:max-h-none lg:overflow-visible scroll-y">

                <div class="lg:hidden flex justify-center pt-3 pb-1">
                    <div class="w-10 h-1 rounded-full bg-beige-300"></div>
                </div>
                    ...
                    <div class="p-5 overflow-y-auto scroll-y">
                        <form action="{{ route('profiles.store') }}" method="POST" class="flex flex-col gap-4">
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                @error('role_id')<p class="text-red-400 text-xs">{{ $message }}</p>@enderror
                            </div>
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                @error('branch_id')<p class="text-red-400 text-xs">{{ $message }}</p>@enderror
                            </div>
                ...
                <div class="bg-white rounded-2xl border border-beige-200 shadow-sm px-5 py-4 flex flex-col gap-3 mx-0 mb-4 lg:mb-0">
            ...
            <div class="flex flex-col gap-4 flex-1 overflow-hidden min-w-0">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 overflow-y-auto scroll-y pr-1 flex-1 content-start">
                        ...
                        <div class="profile-card bg-white rounded-2xl border border-beige-200 p-4 flex flex-col gap-3">
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-brown-400 flex items-center justify-center shrink-0">
                                    <span class="font-playfair text-base font-bold text-beige-200">{{ strtoupper(substr($user->name, 0, 1)) }}</span>
                                </div>
                                <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-col gap-1.5">
                                <div class="flex items-center gap-2">
                                    <svg class="w-3.5 h-3.5 text-beige-400 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/>
                                        <circle cx="9" cy="7" r="4"/>
                                    </svg>
                                    <span class="text-xs text-brown-300 font-medium">{{ ucfirst($user->role->name ?? '—') }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <svg class="w-3.5 h-3.5 text-beige-400 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/>
                                ...
                                @if($user->role_id == 3 && $user->shifts->isNotEmpty())
                                    <div class="flex flex-col gap-1 max-h-24 overflow-y-auto scroll-y">
                                        @foreach($user->shifts as $shift)
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="text-xs text-brown-300 font-medium">
                                                    {{ \Carbon\Carbon::parse($shift->created_at)->format('H:i') }} — {{ $shift->ends_at ? \Carbon\Carbon::parse($shift->ends_at)->format('H:i') : 'Active' }}
                                                </span>
                                            </div>
                                        @endforeach
                                    </div>
                    ...
                    @empty
                        <div class="col-span-full flex flex-col items-center justify-center gap-3 py-24 text-beige-400">
                            ...
                            <p class="text-sm text-center">No profiles yet.<br>Create one to get started.</p>
                        </div>
                    @endforelse
...
<script>
    ...
    function toggleCreatePanel() {
        ...
        const overlay = document.getElementById('create-panel-overlay');
        // Only act as bottom sheet on < lg
